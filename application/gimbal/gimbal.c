...
void GimbalInit()
{
    ...
    // PITCH (GM6020): 角度环对电机机械 total_angle 闭环(用于机械限位),速度环用 IMU 陀螺反馈实现 IMU 级稳定
    Motor_Init_Config_s pitch_config = {
        ...
        .controller_param_init_config = {
            ...
            // 速度环用 IMU pitch 角速度(GyroDeg[0])反馈,角度环用电机 total_angle(MOTOR_FEED)
...
void GimbalTask()
{
    ...
    switch (gimbal_cmd_recv.gimbal_mode)
    {
        case GIMBAL_IMU:
        case GIMBAL_LOCK:
        // pitch: IMU 误差转换为电机机械角度增量,对机械 total_angle 限位
        float imu_error_deg = gimbal_cmd_recv.pitch - gimbal_IMU_data->Pitch;
        float pitch_ref = pitch_motor->measure.total_angle + imu_error_deg;

        if (pitch_ref > PITCH_MAX_DEG)
        {
            pitch_ref = PITCH_MAX_DEG;
        }
        else if (pitch_ref < PITCH_MIN_DEG)
        {
            pitch_ref = PITCH_MIN_DEG;
        }
