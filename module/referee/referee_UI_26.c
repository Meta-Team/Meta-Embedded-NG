...
void UIDelete(referee_id_t *_id, uint8_t Del_Operate, uint8_t Del_Layer)
{
	...
	UI_delete_data.CmdID = ID_robot_interaction_data;
				...
				uint32_t Graph_Width, uint32_t Start_x, uint32_t Start_y, uint32_t End_x, uint32_t End_y)
{
	...
	{
		graph->figure_name[2 - i] = graphname[i]; // 按内存地址增大方向填充，所以会有i与2-i
	}

	graph->operate_type = Graph_Operate;
	graph->figure_type = figure_type_Line;
	...
	graph->details_c = 0;
	graph->details_d = End_x;
	graph->details_e = End_y;
}
					 ...
					 uint32_t Graph_Width, uint32_t Start_x, uint32_t Start_y, uint32_t End_x, uint32_t End_y)
{
	...
	{
		graph->figure_name[2 - i] = graphname[i];
	}

	graph->figure_type = figure_type_Rectangle;
	graph->operate_type = Graph_Operate;
	graph->layer = Graph_Layer;
	graph->color = Graph_Color;
				  ...
				  uint32_t Graph_Width, uint32_t Start_x, uint32_t Start_y, uint32_t Graph_Radius)
{
	...
	graph->figure_type = figure_type_Circle;
	graph->operate_type = Graph_Operate;
	graph->layer = Graph_Layer;
	graph->color = Graph_Color;
				...
				uint32_t Graph_Width, uint32_t Start_x, uint32_t Start_y, uint32_t end_x, uint32_t end_y)
{
	...
	graph->figure_type = figure_type_Ellipse;
	graph->operate_type = Graph_Operate;
	graph->layer = Graph_Layer;
	graph->color = Graph_Color;
			   ...
			   uint32_t end_x, uint32_t end_y)
{
	...
	graph->figure_type = figure_type_Arc;
	graph->operate_type = Graph_Operate;
	graph->layer = Graph_Layer;
	graph->color = Graph_Color;
				 ...
				 uint32_t Graph_Size, uint32_t Graph_Digit, uint32_t Graph_Width, uint32_t Start_x, uint32_t Start_y, int32_t Graph_Float)
{
	...
	graph->figure_type = figure_type_Float;
	graph->operate_type = Graph_Operate;
	graph->layer = Graph_Layer;
	graph->color = Graph_Color;
			   ...
			   uint32_t Graph_Size, uint32_t Graph_Width, uint32_t Start_x, uint32_t Start_y, int32_t Graph_Integer)
{
	...
	graph->figure_type = figure_type_Int;
	graph->operate_type = Graph_Operate;
	graph->layer = Graph_Layer;
	graph->color = Graph_Color;
				...
				uint32_t Graph_Size, uint32_t Graph_Width, uint32_t Start_x, uint32_t Start_y, char *fmt, ...)
{
	...
	for (i = 0; i < 3 && graphname[i] != '\0'; i++)
	{
		graph->graphic_data_struct.figure_name[2 - i] = graphname[i];
	}

	graph->graphic_data_struct.figure_type = figure_type_Char;
	graph->graphic_data_struct.operate_type = Graph_Operate;
	graph->graphic_data_struct.layer = Graph_Layer;
	graph->graphic_data_struct.color = Graph_Color;

	graph->graphic_data_struct.width = Graph_Width;
	graph->graphic_data_struct.start_x = Start_x;
	graph->graphic_data_struct.start_y = Start_y;
	graph->graphic_data_struct.details_a = Graph_Size;
	graph->graphic_data_struct.details_c = 0;
	graph->graphic_data_struct.details_d = 0;
	graph->graphic_data_struct.details_e = 0;

	va_list ap;
	va_start(ap, fmt);
	vsprintf((char *)graph->data, fmt, ap); // 使用参数列表进行格式化并输出到字符串
	va_end(ap);
	graph->graphic_data_struct.details_b = strlen((const char *)graph->data);
}
...
void UIGraphRefresh(referee_id_t *_id, int cnt, ...)
{
	...
	UI_GraphReFresh_data.CmdID = ID_robot_interaction_data;
...
void UICharRefresh(referee_id_t *_id, ext_client_custom_character_t string_Data)
{
	...
	UI_CharReFresh_data.CmdID = ID_robot_interaction_data;
